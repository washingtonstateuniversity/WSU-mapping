	...
	$.extend( WSU_MAP||(WSU_MAP={}), {
			...
			(function check_area(){
				setTimeout(
					function(){
						...
						if(WSU_MAP.state.map_jObj.is(':visible') || 1){
		...
		setup: function () {
			...
			if( window._defined( used_url ) && used_url!==null && used_url!=="" ){
				...
				var cats = WSU_MAP.getParameterByName("cat[]", used_url);
				var pid = WSU_MAP.getParameterByName("pid", used_url);
				if (cats)
				    WSU_MAP.updateMap(cats, false);
				if (pid)
				    WSU_MAP.updateMap(pid, false);

				WSU_MAP.state.map_jObj.on('wsu_maps:data_loaded',function(){
				    if (parseInt(pid, 10) > 0) {
				        var marker = WSU_MAP.state.markerbyid[parseInt(pid, 10)];
						$(marker).triggerEvent('click');
				    }
                    // center
				});	
				
				if( cats && cats.indexOf(',')>0 ){
					$.each( cats.split(','), function(i,v){
						WSU_MAP.nav.menuDressChild($('#main_nav a[href$="'+v+'"]'));
					});
				}else{
					WSU_MAP.nav.menuDressChild($('#main_nav a[href$="'+cats+'"]'));
				}		
			}
			...
			$('#printPdfs').off().on("click",function(e){
				...
				WSU_MAP.util.popup_message({
					...
					html:'<div id="printPdfs">'+
									...
									'<div><h3><a href="http://transportation.wsu.edu/docs/map.pdf" target="_blank">Parking<br/><span id="parking" style="background-image:url('+WSU_MAP.state.siteroot+'Content/images/print/parking_icon.jpg);"></span></a></h3></div>'+
		...
		on_zoom_corrections:function(){
		   /* google.maps.event.addListener(WSU_MAP.state.map_inst, 'zoom_changed', function () {
				...
				WSU_MAP.poi_rest();
				WSU_MAP.set_center();
				window._i("leaving on_zoom_corrections", WSU_MAP.state.map_inst.get("zoom"));
			});*/
		},
