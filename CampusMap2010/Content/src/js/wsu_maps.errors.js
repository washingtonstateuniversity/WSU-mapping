		...
		addErrorReporting:function (marker){
			$('.errorReporting').off().on("click",function(e){
				...
				$.wsu_maps.util.popup_message({
					html:'<div id="errorReporting"><form action="'+$.wsu_maps.state.siteroot+'/public/reportError.castle" method="post">'+
						...
						$('#errorReporting [type="Submit"]').off().on('click',function(e){
							...
							if(valid){
								$.post($('#errorReporting form').attr('action'), $('#errorReporting form').serialize(),function(){
									...
									$('#errorReporting').html('<h3>Thank you for reporting the error.</h3><input type="Submit" id="errorClose" value="Close"/>');
									$('#errorClose').off().on("click",function(e){
										e.stopPropagation();
										e.preventDefault();
										jObj.dialog( "close" );
									});
								...
								if($('#valid').length===0){
									$('#errorReporting').prepend("<div id='valid'><h4 class='error'>Please completely fill out the form so we may completely help you.</h4></div>");
								}
							}
						});
					}
