var FileName = "";
var placeImgBtn = $.wsu_maps.admin.defaults.place_id > 0 ? "|,mainImage,|,imagegallery," : "";
...
$.wsu_maps.admin.ui.tinymce = {
		...
		$("#dialog-pickimage").dialog({
			...
			create: function() { //event, ui) {
				...
				$('#imagePicker').on('change', function() {
					...
					$('#imgPre img').attr('src', $.wsu_maps.state.siteroot + 'media/download.castle?id=' + imgid + '&placeid=' + $.wsu_maps.admin.defaults.place_id + '&m=crop&w=150&h=150&pre=TMP');
				...
				$('#ISIUarea').load($.wsu_maps.state.siteroot + 'media/inlineupload.castle', function() {
					...
					$('input[type=file]').ajaxfileupload({
						'action': $.wsu_maps.state.siteroot + 'media/uploadFiles.castle',
						'params': {
							'returnType': 'id',
							'pool': 'place',
							'pool_place': $.wsu_maps.admin.defaults.place_id,
						...
						'onComplete': function(response) {
							...
							if ($.wsu_maps.util.isNumber(response)) {
								...
								$('#imgPre img').attr('src', $.wsu_maps.state.siteroot + 'media/download.castle?id=' + image_id + '&placeid=' + $.wsu_maps.admin.defaults.place_id + '&m=crop&w=150&h=150&pre=TMP');
								$('#imgPre img').load(function() {
									$('#imgPre img').css({
										'opacity': '1.0'
									});
								...
								$('#place').on('click', function(e) {
									...
									ed.selection.execCommand('mceInsertContent', false, $.wsu_maps.admin.ui.tinymce.get_TinyMCE_InlinImage(image_id));
		...
		$("#dialog-pickimage").dialog({
			...
			create: function() { //event, ui) {
				...
				$('#imagePicker').on('change', function() {
					...
					var imgid = $('#imagePicker :selected').val();
					var img_src = $.wsu_maps.state.siteroot + 'media/download.castle?id=' + imgid + '&placeid=' + $.wsu_maps.admin.defaults.place_id + '&m=crop&w=150&h=150&pre=TMP';
					...
					$('input[type=file]').ajaxfileupload({
						...
						'onComplete': function(response) {
							...
							if ($.wsu_maps.util.isNumber(response)) {
								...
								$('#imagePicker :selected').attr('selected', false); //reset selection
								$('#imagePicker option:first').after('<option value="' + image_id + '" selected="selected">' + FileName + '</option>'); // add new and select
								$('#imgPre img').attr('src', 'media/download.castle?id=' + image_id + '&placeid=' + $.wsu_maps.admin.defaults.place_id + '&m=crop&w=150&h=150&pre=TMP');
								$('#imgPre img').load(function() {
									$('#imgPre img').css({
										'opacity': '1.0'
									});
			...
			buttons: {
				"Insert Image": function() {
					if ($('#imagePicker :selected').val() !== '') {
						tinyMCE.activeEditor.execCommand('mceInsertContent', false, $.wsu_maps.admin.ui.tinymce.get_TinyMCE_InlinImage($('#imagePicker :selected').val()));
						$(this).dialog("close");
					} else {
	...
	contextmenu_fix: function() { //ed) {
		...
		tinyMCE.activeEditor.plugins.contextmenu.onContextMenu.add($.wsu_maps.admin.ui.tinymce.editor_remove_insertImage);
	},
					...
					handle_node_change_callback: function(ed, node) { //, undo_index, undo_levels, visual_aid, any_selection){
						...
						if (window._defined(imgNode.attr('src'))) {
							...
							if (image[0] === "imagingIt") {
								...
								imgNode.attr('title', '#Inline_Iamge(' + image_id + ' ' + $.wsu_maps.admin.defaults.place_id + ' ' + imgNode.width() + ' ' + imgNode.height() + ' \'' + theclass[1] + '\')');
								$.wsu_maps.admin.ui.tinymce.contextmenu_fix(ed);
							}
							...
							if (image[0] === "gallerya") {
								//var image_id = image[1];
								imgNode.attr('title', '#imagegallery(\'' + $.wsu_maps.admin.defaults.place_id + '\' ' + imgNode.width() + ' ' + imgNode.height() + ' \'' + theclass[1] + '\')');
							}
						}
					},
					setup: function(ed) {
						...
						ed.onInit.add($.wsu_maps.admin.ui.tinymce.editor_oninit);
						// Add a custom button
						ed.addButton('mainImage', {
							title: 'Main Image',
							image: $.wsu_maps.state.siteroot + 'Content/images/insert_image.png',
							onclick: function() {
								// Add you own code to execute something on click
								ed.focus();
								$.wsu_maps.admin.ui.tinymce.openImgResource(ed);
							}
						});
						ed.addButton('youTube', {
							title: 'Add YouTube',
							image: $.wsu_maps.state.siteroot + 'Content/images/insert_youtube.png',
							onclick: function() {
								// Add you own code to execute something on click
								ed.focus();
								...
								$("#dialog-youtube").dialog({
									...
									buttons: {
										"Set Image": function() {
											...
											tinyMCE.activeEditor.execCommand('mceInsertContent', true, $.wsu_maps.admin.ui.tinymce.get_TinyMCE_InlinYouTube(yCode, yTitle, w, h, yclass));
										},
										Cancel: function() {
											$(this).dialog("close");
										}
						...
						ed.addButton('imagegallery', {
							title: 'Add Gallery',
							image: $.wsu_maps.state.siteroot + 'Content/images/insert_gallery.png',
							onclick: function() {
								// Add you own code to execute something on click
								ed.focus();
								var w = 250;
								var h = 250;
								var yclass = '';
								tinyMCE.activeEditor.execCommand('mceInsertContent', false, $.wsu_maps.admin.ui.tinymce.get_TinyMCE_imagegallery(w, h, yclass));
	...
	editor_oninit:function (ed) {
		// Add hook for onContextMenu so that Insert Image can be removed
		ed.plugins.contextmenu.onContextMenu.add($.wsu_maps.admin.ui.tinymce.editor_remove_insertImage);
		if ($.wsu_maps.admin.defaults.place_id > 0 && $('#place_Bodytext').val() !== '') {
			setTimeout(function() {
	...
	editor_remove_insertImage:function (sender, menu) {
		...
		for (var itemName in menu.items) {
			...
			if (/^mce_/.test(itemName)) {
				if (mitem.settings) {
					if (mitem.settings.cmd === "mceImage" || mitem.settings.cmd === "mceAdvImage" || ($.wsu_maps.admin.ui.tinymce.isIMG() && mitem.settings.title === 'Edit image')) {
						// skip these items
						continue;
					}
				}
	...
	get_TinyMCE_InlinImage:function (image_id) {
		var baseurl = '/';
		var MACRO = '#Inline_Iamge(' + image_id + ' ' + $.wsu_maps.admin.defaults.place_id + ' 250 250 "")';
		return '<img src="' + baseurl + 'media/download.castle?id=' + image_id + '&placeid=' + $.wsu_maps.admin.defaults.place_id + '&m=crop&w=250&h=250&pre=TMP" class="tinyImgHolder" alt="imagingIt|' + image_id + '" title="' + MACRO + '" />';
	},
	...
	get_TinyMCE_imagegallery:function (w, h, yclass) {
		...
		var baseurl = '/';
		var MACRO = "#imagegallery('" + $.wsu_maps.admin.defaults.place_id + "' " + w + " " + h + " '" + yclass + "')";
		var html = '<img src="' + baseurl + 'media/download.castle?id=' + image_id + '&placeid=' + $.wsu_maps.admin.defaults.place_id + '&m=crop&w=250&h=250&pre=TMP&mark=SLIDESHOW" class="tinyImgHolder" alt="gallerya|' + image_id + '" title="' + MACRO + '"/>';
		return html;
	}
};
