...
if($('.right_side #twtr-search-widget').length>0){
  new TWTR.Widget({
	...
	if($(".userFlagged").length>0){
	    $(".userFlagged").live("click", function(e){
			if($(this).val()==""){
			    e.preventDefault();
			    e.stopPropagation();
	...
	$('.selectedImage').live('click',function(e){
		e.preventDefault();
		$('#main_Image').attr('src',$(this).find('img').attr('src'));
	});
		
	
	$('.comments.uFlaged h3').live('click',function(){
		var obj=$(this);
	...
	$('#classified_main h3').live('click',function(){
		var obj=$(this);
