...
( function($) {
	...
	$.extend($.ui.gmap.prototype, {
		...
		addPolyline: function(shapeOptions, callback) {
		...
		addShape: function(shapeType, shapeOptions, callback) {
			...
			if(typeof(shapeOptions.paths)==='string'){
				shapeOptions.paths = self.process_coords(shapeOptions.paths,flip,false);
			}
			if(typeof(shapeOptions.path)==='string'){
				shapeOptions.path = self.process_coords(shapeOptions.path,flip,false);
			}
		...
		process_coords: function(coordinates,flip,reverse_array) {
			...
			if( typeof(google.maps.geometry)!=='undefined' && coordinates.match(/[\||\@|\_|\`]+/i) ){ // only encoded has @ | _ `
				...
				*/
				coordArray = coordinates.match(/\((-?\d+\.\d+\s?,\s?-?\d+\.\d+)\)/gim);
				...
				if ( coordArray !== null) {
					...
					*/
					for (var i = 0; i < coordArray.length; i++ ) { 
						cord=coordArray[i].replace('(','').replace('(','').replace(' ','').split(',');
					...
					if ( coordArray !== null) {
						...
						*/
						for (var j = 0; j < coordArray.length; j++ ) { 
							cord=coordArray[j].replace('  ',' ').replace(',','').split(' ');
							/* cord normalized from
							*	-122.8 47.65 too
							*	myArray[0] = "-122.8"
							*	myArray[1] = "47.653"
						...
						coordArray = coordinates.match(/(-?\d+\.\d+\s?,\s?-?\d+\.\d+)\s?,?\n?/gim);
						...
						if ( coordArray !== null) {
							...
							*/
							...
							for (var k = 0; k < coordArray.length; k++ ) { 
								cord='';
								var l_str=coordArray[k].substring(coordArray[k].length - 1); 
									l_str===',' ? coordArray[k] = coordArray[k].substring(0,coordArray[k].length - 1) : '' ;
								cord=coordArray[k].replace(' ','').split(',');
								/* cord normalized from
								*	-122.8,47.65, too
								*	myArray[0] = "-122.8"
								*	myArray[1] = "47.653"
		...
		move_shape: function(shape,latlng, callback){
			...
			if (latlng.lng() < currentCenter.lng()) {
				xoff *= -1;
			} 
			if (latlng.lat() < currentCenter.lat()) {
				yoff *= -1;
			} 
			...
			for (var j = 0; j < MVCArray_latLng.length; j++) { 
				var old = MVCArray_latLng[j];
