...
alter table style_events  drop constraint FKE3019E4AC15430A6
...
alter table style_options  drop constraint FKAEABE8A7FB0CCE31
...
if exists (select * from dbo.sysobjects where id = object_id(N'style_option_types_to_geometrics_to_types') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table style_option_types_to_geometrics_to_types
...
if exists (select * from dbo.sysobjects where id = object_id(N'geometrics_to_style') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table geometrics_to_style
...
if exists (select * from dbo.sysobjects where id = object_id(N'style_events') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table style_events
...
create table style_option_types_to_geometrics_to_types (
...
create table geometrics_to_style (
...
create table style_events (
  style_event_id INT IDENTITY NOT NULL,
   name NVARCHAR(255) null,
   friendly_name NVARCHAR(255) null,
   type INT null,
   primary key (style_event_id)
)
create table logs (
  Id INT IDENTITY NOT NULL,
...
alter table style_events  add constraint FKE3019E4AC15430A6 foreign key (type) references geometrics_types 
...
alter table style_options  add constraint FKAEABE8A7FB0CCE31 foreign key (user_event) references style_events 
