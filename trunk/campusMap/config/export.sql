...
alter table style  drop constraint FKEFB8BAC5C15430A6
...
if exists (select * from dbo.sysobjects where id = object_id(N'style') and OBJECTPROPERTY(id, N'IsUserTable') = 1) drop table style
...
create table styles (
  styles_id INT IDENTITY NOT NULL,
   name NVARCHAR(255) null,
   attr NVARCHAR(255) null,
   model NVARCHAR(255) null,
   fieldset INT null,
...
alter table style  add constraint FKEFB8BAC5C15430A6 foreign key (type) references styles 
